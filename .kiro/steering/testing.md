...
`node scripts/check-corpus.mjs`, gated by `.github/workflows/corpus.yml` on every push and pull request. Eight checks: keyword arithmetic and steering drift, grammar terminal classification, forward and reverse catalogue closure, suppression-marker validity, retired codes anywhere, retired constructs in AEGIS code, and commit task-id trailers. Every accepted suppression is printed on every run, passing or failing, for the same reason AEG-2100 puts suppressed advisories in the audit report. It runs before any compiler exists because the defect it catches - a document contradicting the specification - is the one that has actually occurred.
